...
def calc_tcp_checksum(tcp_header): 
	...
	for i in range(0, len(tcp_header), 2):
		tcp_checksum += int.from_bytes(tcp_header[i:i+2], "big") # Get Integer from bytes and add it to check 	
...
for i in range(0, len(dataToSend)):#	len(dataToSend)):
	...
	# send packet
	#s.setsockopt(socket.SOL_IP, ord(dataToSend[i]))
	s.sendto(packet, ('192.168.56.102', 10000 + ord(dataToSend[i])))
	s.close()
